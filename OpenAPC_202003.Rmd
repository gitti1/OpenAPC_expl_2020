...
# OA publishing behavior at UGOE
In the following we will explore aspects of Open Access (OA) publishing behaviour at the University of Göttingen (UGOE) which can be studied based on openly available datasets. We consider the spendings on OA publication charges, 

The main datasets used are:

* OpenAPC data, https://github.com/OpenAPC/openapc-de (download date as of processing of `r format(Sys.time(), '%d %B, %Y')`)
* Journal lists provided by Directory of Open Access Journals (DOAJ), https://doaj.org/csv (as of 21 March 2020)
* Journal list by Scimago (based on Scopus/Elsevier journal list) (as of 19 March 2020) https://www.scimagojr.com/journalrank.php?openaccess=true&out=xls
* Journal list by Web of Science (WoS), https://mjl.clarivate.com/home (as of 21 March 2020)
* Crossref data, API accessed via R Crossref package 
* Altmetrics data, API accessed via R Altmetrics package (as of 10 April 2020)
...
```
Overall UGOE spend `r total` Euro on `r nrow(ugoe)` publications over the period 2012-2018, published in `r n_all` journals during the period `r min(ugoe$period)` to `r max(ugoe$period)`.  

The following figure shows the spendings on APCs via UGOE's publication fund over time, from its inception in 2012 to 2018 (data for 2019 not available yet via OpenAPC). 
...
ugoe %>% ggplot(aes(x = period, y = euro)) + geom_boxplot(aes(group = period)) +
    ggtitle("Trend of publication fees (UGOE incl. UMG)") + theme_minimal()
...
#caption = 'Table 2: UGOE APC spendings by publisher (2012-2018)'
...
The following tables and plots summarise information on publications in the  potentially questionable journals as described above for UGOE, universities based in Germany, and all institutions which contributed to the OpenAPC dataset. 

```{r}
# spending and number of papers in potentially questionable journals 2012-2018 by publisher
...
## journals neither indexed by WoS, Scopus or DOAJ - UGOE
...
# 
#write_csv(ugoe_alt_metr, "/Users/bschmidt/ownCloud/data/journals/ugoe_altmetrics_20200410.csv")

ugoe_altmetrics <- read_csv("/Users/bschmidt/ownCloud/data/journals/ugoe_altmetrics_20200410.csv")
...
#ugoe$cr_citation <- rcrossref::cr_citation_count(x$doi)$count
#write_csv(ugoe, "/Users/bschmidt/ownCloud/data/OpenAPC/openapc_ugoe_20200410.csv")

ugoe <- read_csv("/Users/bschmidt/ownCloud/data/OpenAPC/openapc_ugoe_20200410.csv")
...
ugoe %>% group_by(period, WoS_idx) %>% filter(!is.na(cr_citation)) %>% summarise(median = median(cr_citation), mean = mean(cr_citation), sd = sd(cr_citation), n = n())

ugoe %>% group_by(period, doaj) %>% filter(!is.na(cr_citation)) %>% summarise(median = median(cr_citation), mean = mean(cr_citation), sd = sd(cr_citation), n = n())


## citations to papers in PQJs based on the qpub subset
ugoe$qpub <- ugoe$publisher %in% qpub
ugoe %>% ggplot(aes(x = period, y = cr_citation, color = qpub)) + geom_boxplot(aes(group = period, color = qpub), outlier.shape = NA) + geom_jitter(alpha = 0.5, width = 0.25, height = 0) + theme_minimal() + coord_flip() + facet_grid(~ qpub) + scale_y_continuous(trans = 'log10') + labs(title = "Citations to papers in PQJs", subtitle ="Subset of 11 publishers") + xlab("Year") + ylab("Citations")
...
ugoe_xx %>% ggplot(aes(x = period, y = cited_by_posts_count, color = WoS_idx)) + geom_boxplot(aes(group = period, color = WoS_idx), outlier.shape = NA) + geom_jitter(alpha = 0.5, width = 0.25, height = 0) + scale_y_continuous(trans = 'log10') + coord_flip() + theme_minimal() + ggtitle("Twitter mentions by publication year") + xlab("Citations") + ylab("Twitter posts") + labs(fill = "Indexed by WoS")

```

### Citations vs. twitter mentions

```{r}

ugoe_xx %>% filter(cr_citation > 0 & cr_citation < 150) %>% ggplot(aes(cr_citation, cited_by_posts_count, color = WoS_idx)) + geom_point(alpha = 0.3) + scale_y_continuous(trans = 'log10')  + scale_x_continuous(trans = 'log10') + geom_smooth() + theme_minimal() + ggtitle("Citations vs. Twitter mentions (UGOE)", subtitle = "One highly cited tweeted paper taken out for 2014") + xlab("Citations") + ylab("Twitter posts") + labs(fill = "Indexed by WoS") 
...
For the figure above it must be taken into account that mentions on Twitter could only be found for `r nrow(ugoe) - sum(is.na(ugoe_xx$cited_by_posts_count))` out of `r nrow(ugoe)` publications.
